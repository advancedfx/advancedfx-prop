...
#ifndef SOURCESDK_BM_CONVAR_H
#define SOURCESDK_BM_CONVAR_H
...
//#include "color.h"
#include <bm/sdk_src/public/icvar.h>
...
#define SOURCESDK_BM_COMMAND_COMPLETION_MAXITEMS		64
#define SOURCESDK_BM_COMMAND_COMPLETION_ITEM_LENGTH	64

//-----------------------------------------------------------------------------
// Returns 0 to COMMAND_COMPLETION_MAXITEMS worth of completion strings
//-----------------------------------------------------------------------------
typedef int  ( *FnCommandCompletionCallback )( const char *partial, char commands[SOURCESDK_BM_COMMAND_COMPLETION_MAXITEMS ][SOURCESDK_BM_COMMAND_COMPLETION_ITEM_LENGTH ] );
...
public:
	...
	bool Tokenize( const char *pCommand, characterset_t *pBreakSet = SOURCESDK_NULL );
	void Reset();
...
{
	return m_nArgc ? (const char**)m_ppArgv : SOURCESDK_NULL;
}
...
public:
	...
	// Retrieve value
	SOURCESDK_FORCEINLINE_CVAR float			GetFloat( void ) const;
	SOURCESDK_FORCEINLINE_CVAR int			GetInt( void ) const;
	SOURCESDK_FORCEINLINE_CVAR Color			GetColor( void ) const;
	SOURCESDK_FORCEINLINE_CVAR bool			GetBool() const {  return !!GetInt(); }
	SOURCESDK_FORCEINLINE_CVAR char const	   *GetString( void ) const;
...
// Output : float
//-----------------------------------------------------------------------------
SOURCESDK_FORCEINLINE_CVAR float ConVar::GetFloat( void ) const
{
...
// Output : int
//-----------------------------------------------------------------------------
SOURCESDK_FORCEINLINE_CVAR int ConVar::GetInt( void ) const
{
#ifdef CONVAR_TEST_MATERIAL_THREAD_CONVARS
	Assert( ThreadInMainThread() || IsFlagSet( FCVAR_MATERIAL_THREAD_MASK | FCVAR_ACCESSIBLE_FROM_THREADS ) );
#endif
...
// Output : Color
//-----------------------------------------------------------------------------
//SOURCESDK_FORCEINLINE_CVAR Color ConVar::GetColor( void ) const
//{
//#ifdef CONVAR_TEST_MATERIAL_THREAD_CONVARS
//	Assert( ThreadInMainThread() || IsFlagSet( FCVAR_MATERIAL_THREAD_MASK | FCVAR_ACCESSIBLE_FROM_THREADS ) );
//#endif
...
template <> SOURCESDK_FORCEINLINE_CVAR float			ConVar::Get<float>( void ) const		{ return GetFloat(); }
template <> SOURCESDK_FORCEINLINE_CVAR int			ConVar::Get<int>( void ) const			{ return GetInt(); }
template <> SOURCESDK_FORCEINLINE_CVAR bool			ConVar::Get<bool>( void ) const			{ return GetBool(); }
template <> SOURCESDK_FORCEINLINE_CVAR const char *	ConVar::Get<const char *>( void ) const	{ return GetString(); }
template <> SOURCESDK_FORCEINLINE_CVAR float			ConVar::Get<float>( float *p ) const				{ return ( *p = GetFloat() ); }
template <> SOURCESDK_FORCEINLINE_CVAR int			ConVar::Get<int>( int *p ) const					{ return ( *p = GetInt() ); }
template <> SOURCESDK_FORCEINLINE_CVAR bool			ConVar::Get<bool>( bool *p ) const					{ return ( *p = GetBool() ); }
template <> SOURCESDK_FORCEINLINE_CVAR const char *	ConVar::Get<const char *>( char const **p ) const	{ return ( *p = GetString() ); }

//-----------------------------------------------------------------------------
// Purpose: Return ConVar value as a string, return "" for bogus string pointer, etc.
// Output : const char *
//-----------------------------------------------------------------------------
SOURCESDK_FORCEINLINE_CVAR const char *ConVar::GetString( void ) const
{
#ifdef CONVAR_TEST_MATERIAL_THREAD_CONVARS
	Assert( ThreadInMainThread() || IsFlagSet( FCVAR_MATERIAL_THREAD_MASK | FCVAR_ACCESSIBLE_FROM_THREADS ) );
#endif
	if ( m_nFlags & SOURCESDK_BM_FCVAR_NEVER_AS_STRING )
...
//			( pBaseVar->GetFlags() & ~SOURCESDK_BM_FCVAR_SS ) | SOURCESDK_BM_FCVAR_SS_ADDED,
...
//		SOURCESDK_Assert( IsFlagSet(SOURCESDK_BM_FCVAR_SS_ADDED ) );
//		SOURCESDK_Assert( !IsFlagSet(SOURCESDK_BM_FCVAR_SS ) );
//	}
//
...
//-----------------------------------------------------------------------------
SOURCESDK_FORCEINLINE_CVAR bool ConVarRef::IsFlagSet( int nFlags ) const
{
	return ( m_pConVar->IsFlagSet( nFlags ) != 0 );
}

SOURCESDK_FORCEINLINE_CVAR IConVar *ConVarRef::GetLinkedConVar()
{
	return m_pConVar;
}

SOURCESDK_FORCEINLINE_CVAR const char *ConVarRef::GetName() const
{
...
SOURCESDK_FORCEINLINE_CVAR int ConVarRef::GetSplitScreenPlayerSlot() const
{
...
// Purpose: Return ConVar value as a float
//-----------------------------------------------------------------------------
SOURCESDK_FORCEINLINE_CVAR float ConVarRef::GetFloat( void ) const
{
...
// Purpose: Return ConVar value as an int
//-----------------------------------------------------------------------------
SOURCESDK_FORCEINLINE_CVAR int ConVarRef::GetInt( void ) const
{
...
// Purpose: Return ConVar value as a color
//-----------------------------------------------------------------------------
//SOURCESDK_FORCEINLINE_CVAR Color ConVarRef::GetColor( void ) const
//{
...
{
	SOURCESDK_Assert( !IsFlagSet( SOURCESDK_BM_FCVAR_NEVER_AS_STRING ) );
	return m_pConVarState->m_Value.m_pszString;
}


SOURCESDK_FORCEINLINE_CVAR void ConVarRef::SetValue( const char *pValue )
{
	m_pConVar->SetValue( pValue );
}

SOURCESDK_FORCEINLINE_CVAR void ConVarRef::SetValue( float flValue )
{
	m_pConVar->SetValue( flValue );
}

SOURCESDK_FORCEINLINE_CVAR void ConVarRef::SetValue( int nValue )
{
	m_pConVar->SetValue( nValue );
}

//SOURCESDK_FORCEINLINE_CVAR void ConVarRef::SetValue( Color value )
//{
//	m_pConVar->SetValue( value );
//}

SOURCESDK_FORCEINLINE_CVAR void ConVarRef::SetValue( bool bValue )
{
	m_pConVar->SetValue( bValue ? 1 : 0 );
}

SOURCESDK_FORCEINLINE_CVAR const char *ConVarRef::GetDefault() const
{
...
//-----------------------------------------------------------------------------
#define SOURCESDK_BM_CON_COMMAND( name, description ) \
   static void name( const CCommand &args ); \
   static ConCommand name##_command( #name, name, description ); \
   static void name( const CCommand &args )

#ifdef SOURCESDK_CLIENT_DLL
	#define CON_COMMAND_SHARED( name, description ) \
		static void name( const CCommand &args ); \
		static ConCommand name##_command_client( #name "_client", name, description ); \
		static void name( const CCommand &args )
#else
	#define SOURCESDK_BM_CON_COMMAND_SHARED( name, description ) \
		static void name( const CCommand &args ); \
		static ConCommand name##_command( #name, name, description ); \
		static void name( const CCommand &args )
#endif
...
#define SOURCESDK_BM_CON_COMMAND_F( name, description, flags ) \
	static void name( const CCommand &args ); \
	...
	#define CON_COMMAND_F_SHARED( name, description, flags ) \
		...
		static void name( const CCommand &args )
#else
	#define SOURCESDK_BM_CON_COMMAND_F_SHARED( name, description, flags ) \
		static void name( const CCommand &args ); \
		static ConCommand name##_command( #name, name, description, flags ); \
		static void name( const CCommand &args )
#endif
...
#define SOURCESDK_BM_CON_COMMAND_F_COMPLETION( name, description, flags, completion ) \
	static void name( const CCommand &args ); \
	static ConCommand name##_command( #name, name, description, flags, completion ); \
	static void name( const CCommand &args )

#ifdef SOURECESDK_CLIENT_DLL
	#define CON_COMMAND_F_COMPLETION_SHARED( name, description, flags, completion ) \
		...
		static void name( const CCommand &args )
#else
	#define SOURCESDK_BM_CON_COMMAND_F_COMPLETION_SHARED( name, description, flags, completion ) \
		static void name( const CCommand &args ); \
		static ConCommand name##_command( #name, name, description, flags, completion ); \
		static void name( const CCommand &args )
#endif
...
#define SOURCESDK_BM_CON_COMMAND_EXTERN( name, _funcname, description ) \
	void _funcname( const CCommand &args ); \
	static ConCommand name##_command( #name, _funcname, description ); \
	void _funcname( const CCommand &args )

#define SOURCESDK_BM_CON_COMMAND_EXTERN_F( name, _funcname, description, flags ) \
	void _funcname( const CCommand &args ); \
	static ConCommand name##_command( #name, _funcname, description, flags ); \
	void _funcname( const CCommand &args )

#define SOURCESDK_BM_CON_COMMAND_MEMBER_F( _thisclass, name, _funcname, description, flags ) \
	void _funcname( const CCommand &args );						\
