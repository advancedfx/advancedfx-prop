...
#include <garrysmod/sdk_src/public/vstdlib/IKeyValuesSystem.h>
...
KeyValues *KeyValues::FindKey(const char *keyName, bool bCreate)
{
	...
	if (subStr)
	{
		...
		SOURCESDK_Q_memcpy( szBuf, keyName, size );
		szBuf[size] = 0;
		searchStr = szBuf;
	}
...
void KeyValues::SetStringValue( char const *strValue )
{
	...
	SOURCESDK_Q_memcpy( m_sValue, strValue, len+1 );
...
void KeyValues::SetString( const char *keyName, const char *value )
{
	...
	if ( dat )
	{
		...
		SOURCESDK_Q_memcpy( dat->m_sValue, value, len+1 );
...
void KeyValues::SetWString( const char *keyName, const wchar_t *value )
{
	...
	if ( dat )
	{
		...
		SOURCESDK_Q_memcpy( dat->m_wsValue, value, (len+1) * sizeof(wchar_t) );
		...
		case TYPE_UINT64:
			{
				...
				SOURCESDK_Q_memcpy( m_sValue, src.m_sValue, sizeof(uint64) );
			}
			break;
		case TYPE_COLOR:
			{
...
KeyValues *KeyValues::MakeCopy( void ) const
{
	...
	case TYPE_STRING:
		{
			if ( m_sValue )
			{
				...
				SOURCESDK_Q_memcpy( newKeyValue->m_sValue, m_sValue, len+1 );
			}
		}
		break;
	case TYPE_WSTRING:
		{
			if ( m_wsValue )
			{
				...
				SOURCESDK_Q_memcpy( newKeyValue->m_wsValue, m_wsValue, (len+1)*sizeof(wchar_t));
			}
		}
		break;
	...
	case TYPE_UINT64:
		...
		SOURCESDK_Q_memcpy( newKeyValue->m_sValue, m_sValue, sizeof(uint64) );
		break;
	};
		...
		case TYPE_STRING:
			{
				...
				dat->m_sValue = KVStringAlloc<char>(len + 1);
				SOURCESDK_Q_memcpy( dat->m_sValue, token, len+1 );
