...
void Addresses_InitStdshader_dx9Dll(AfxAddr stdshader_dx9Dll, bool isCsgo)
{
	if(isCsgo)
	{
		...
		// csgo_UnlitGeneric_CShader_OnDrawElements:
		{
			...
			if(addr)
			{
				AFXADDR_SET(csgo_UnlitGeneric_CShader_OnDrawElements, addr);
			}
			else
			{
		...
		// csgo_VertexLitGeneric_CShader_OnDrawElements:
		{
			DWORD addr = 0;
			{
				ImageSectionsReader sections((HMODULE)stdshader_dx9Dll);
			...
			if(addr)
			{
				AFXADDR_SET(csgo_VertexLitGeneric_CShader_OnDrawElements, addr);
			}
			else
			{
		...
		*/
		...
		// csgo_stdshader_dx9_CBaseShader_DrawElements:
		{
			...
			if(addr)
			{
				AFXADDR_SET(csgo_stdshader_dx9_CBaseShader_DrawElements, addr);
			}
			else
			{
