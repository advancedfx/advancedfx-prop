...
#ifndef SOURCESDK_CS2_UTLMEMORY_H
#define SOURCESDK_CS2_UTLMEMORY_H
...
#ifndef SOURCESDK_CS2_SIZE_MAX
#define SOURCESDK_CS2_SIZE_MAX ((size_t)-1)
#endif
...
#ifdef SOURCESDK_CS2_UTLMEMORY_TRACK
#define SOURCESDK_CS2_UTLMEMORY_TRACK_ALLOC()		MemAlloc_RegisterAllocation( "Sum of all UtlMemory", 0, m_nAllocationCount * sizeof(T), m_nAllocationCount * sizeof(T), 0 )
#define SOURCESDK_CS2_UTLMEMORY_TRACK_FREE()		if ( !m_pMemory ) ; else MemAlloc_RegisterDeallocation( "Sum of all UtlMemory", 0, m_nAllocationCount * sizeof(T), m_nAllocationCount * sizeof(T), 0 )
#else
#define SOURCESDK_CS2_UTLMEMORY_TRACK_ALLOC()		((void)0)
#define SOURCESDK_CS2_UTLMEMORY_TRACK_FREE()		((void)0)
#endif
...
public:
	...
	{
		SOURCESDK_Assert( nInitSize == 0 || nInitSize == SIZE );
	}
...
public:
	// constructor, destructor
	CUtlMemoryFixed( int nGrowSize = 0, int nInitSize = 0 )	{ SOURCESDK_Assert( nInitSize == 0 || nInitSize == SIZE ); 	}
	CUtlMemoryFixed( T* pMemory, int numElements )			{ SOURCESDK_Assert( 0 ); 										}
	...
	// element access
	T& operator[]( int i )									{ SOURCESDK_Assert( IsIdxValid(i) ); return Base()[i];	}
	const T& operator[]( int i ) const						{ SOURCESDK_Assert( IsIdxValid(i) ); return Base()[i];	}
	T& Element( int i )										{ SOURCESDK_Assert( IsIdxValid(i) ); return Base()[i];	}
	const T& Element( int i ) const							{ SOURCESDK_Assert( IsIdxValid(i) ); return Base()[i];	}

	// Attaches the buffer to external memory....
	void SetExternalBuffer( T* pMemory, int numElements )	{ SOURCESDK_Assert( 0 ); }
	...
	// Grows the memory, so that at least allocated + num elements are allocated
	void Grow( int num = 1 )								{ SOURCESDK_Assert( 0 ); }

	// Makes sure we've got at least this much memory
	void EnsureCapacity( int num )							{ SOURCESDK_Assert( num <= SIZE ); }

	// Memory deallocation
	void Purge()											{}

	// Purge all but the given number of elements (NOT IMPLEMENTED IN CUtlMemoryFixed)
	void Purge( int numElements )							{ SOURCESDK_Assert( 0 ); }
...
public:
	...
	CUtlMemoryConservative( T* pMemory, int numElements )								{ SOURCESDK_Assert( 0 ); }
	...
	// Size
	SOURCESDK_FORCEINLINE void RememberAllocSize( size_t sz )
	{
#ifdef REMEMBER_ALLOC_SIZE_FOR_VALGRIND
		m_nCurAllocSize = sz;
#endif
	...
	SOURCESDK_FORCEINLINE void ReAlloc( size_t sz )
	{
...
{
	SOURCESDK_Assert( !IsReadOnly() );
	SOURCESDK_Assert( IsIdxValid(i) );
	return m_pMemory[i];
}
...
{
	SOURCESDK_Assert( numElements >= 0 );

	if( numElements > m_nAllocationCount )
	{
		// Ensure this isn't a grow request in disguise.
		SOURCESDK_Assert( numElements <= m_nAllocationCount );
		return;
	}
...
CUtlMemoryAligned<T, nAlignment>::CUtlMemoryAligned( int nGrowSize, int nInitAllocationCount )
{
	...
	SOURCESDK_Assert( (nGrowSize >= 0) && (nGrowSize & CUtlMemory<T>::EXTERNAL_BUFFER_MARKER) == 0 );
...
void CUtlMemoryAligned<T, nAlignment>::Grow( int num )
{
	...
	if ( CUtlMemory<T>::m_pMemory )
	{
		...
		SOURCESDK_Assert( CUtlMemory<T>::m_pMemory );
	}
	else
	{
		//TODO//MEM_ALLOC_CREDIT_CLASS();
...
public:
	...
	CUtlMemory_RawAllocator( T* pMemory, int numElements ) { SOURCESDK_Assert( 0 ); }
...
#endif // SOURCESDK_CS2_UTLMEMORY_H
