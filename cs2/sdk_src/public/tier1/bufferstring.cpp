...
const char * CBufferString::Insert(int nIndex, const char *pBuf, int nCount, bool bIgnoreAlignment) {
    ...
    auto pFn = (Insert_t)GetProcAddress(GetTier0DllHandle(),"?Insert@CBufferString@@QEAAPEBDHPEBDH_N@Z");
...
void CBufferString::Purge(int nAllocatedBytesToPreserve) {
    ...
    auto pFn = (Insert_t)GetProcAddress(GetTier0DllHandle(),"?Purge@CBufferString@@QEAAXH@Z");
...
const char * CBufferString::FixupPathName(char cSeparator) {
    ...
    auto pFn = (FixupPathName_t)GetProcAddress(GetTier0DllHandle(),"?FixupPathName@CBufferString@@QEAAPEBDD@Z");
...
void CBufferString::ToLowerFast(int nStart) {
    ...
    auto pFn = (ToLowerFast_t)GetProcAddress(GetTier0DllHandle(),"?ToLowerFast@CBufferString@@QEAAXH@Z");
...
const char * CBufferString::FixSlashes(char cSeparator) {
    ...
    auto pFn = (FixSlashes_t)GetProcAddress(GetTier0DllHandle(),"?FixSlashes@CBufferString@@QEAAPEBDD@Z");
    if(pFn) return pFn(this, cSeparator);
	return "";
}
...
const char * CBufferString::ExtractFileExtension(const char *pPath) {
    ...
    auto pFn = (ExtractFileExtension_t)GetProcAddress(GetTier0DllHandle(),"?ExtractFileExtension@CBufferString@@QEAAPEBDPEBD@Z");
