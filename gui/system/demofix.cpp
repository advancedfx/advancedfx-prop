...
bool CHlaeDemoFix::normal_demo ( const wxChar* infilename, const wxChar* outfilename)
{
	...
	if (infile->IsOpened() && outfile->IsOpened())
	{
		...
		if (!(infile->IsOpened())) g_debug.SendMessage(_T("Could not open input file."),hlaeDEBUG_ERROR);
		if (!(outfile->IsOpened())) g_debug.SendMessage(_T("Could not open output file."),hlaeDEBUG_ERROR);
	}
...
bool CHlaeDemoFix::fix_demo ( const wxChar* infilename, const wxChar* outfilename)
{
	...
	if (infile->IsOpened() && outfile->IsOpened())
	{
		...
		if (!(infile->IsOpened())) g_debug.SendMessage(_T("DemoFix: Could not open input file."),hlaeDEBUG_ERROR);
		if (!(outfile->IsOpened())) g_debug.SendMessage(_T("DemoFix: Could not open output file."),hlaeDEBUG_ERROR);
	}
...
bool CHlaeDemoFix::read_header(wxFile* infile, hldemo_header_s * header)
{
	...
	if (!compare_bytes(header->magic,HLDEMO_MAGIC,sizeof(HLDEMO_MAGIC))) { g_debug.SendMessage(_T("file identifier invalid"),hlaeDEBUG_ERROR); return false; }
...

{
	...
	while (cbCopy)
	{
		...
		{
			g_debug.SendMessage(_T("error when reading from input"),hlaeDEBUG_ERROR);
			bRes=false;
			break;
		}
		...
		{
			g_debug.SendMessage(_T("error when writing to output"),hlaeDEBUG_ERROR);
			bRes=false;
			break;
		}
