...
public:
	...
	void AddCommandMapping ( wxString & srcmap, wxString & targetmap);
	...
	bool Run ( const wxChar* infilename, const wxChar* outfilename);
...
private:
	...
	char _watermark64[64];
	char _watermark260[260];
	...
	std::list <cmd_mapping_s> _CommandMap;

	bool normal_demo ( const wxChar* infilename, const wxChar* outfilename);
	...
	copy_macroblock_e copy_macroblock(wxFile* infile, wxFile* outfile, hldemo_macroblock_header_s* pblock_header = NULL);
	...
	copy_macroblock_e copy_command(wxFile* infile, wxFile* outfile);
	...
	copy_macroblock_e copy_gamedata(wxFile* infile,wxFile* outfile, unsigned int dwreadbytes);
